...
function ScreenshotCarousel() {
    ...
    return (
        <div className="rounded-3xl overflow-hidden border border-[#C0A880]/10 h-80 relative">
            <Image src={screens[current]} alt={`Screenshot ${current + 1}`} width={700} height={400} className="w-full h-full object-cover" />
            {screens.length > 1 && (
                <>
                    <button onClick={prev} className="absolute left-3 top-1/2 -translate-y-1/2 rounded-full bg-[#0A0806]/70 backdrop-blur p-2 hover:bg-[#221408] transition"><ChevronLeft className="h-5 w-5 text-[#F0E8D8]" /></button>
                    <button onClick={next} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full bg-[#0A0806]/70 backdrop-blur p-2 hover:bg-[#221408] transition"><ChevronRight className="h-5 w-5 text-[#F0E8D8]" /></button>
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                        {screens.map((_, i) => <button key={i} onClick={() => setCurrent(i)} className={`w-2 h-2 rounded-full transition ${i === current ? "bg-[#F0E8D8]" : "bg-[#F0E8D8]/30"}`} />)}
                    </div>
                </>
            )}
        </div>
...
function DocumentCard({ doc, onClick }) {
    return (
        <motion.div variants={fadeUp} onClick={onClick} className="rounded-3xl border border-[#C0A880]/10 bg-[#221408]/20 overflow-hidden cursor-pointer hover:border-[#80685A]/40 transition-all group">
            <div className="h-44 bg-[#120A04]/60 overflow-hidden flex items-center justify-center">
                {doc.cover ? <img src={doc.cover} alt={doc.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                    ...
                    : <div className="flex flex-col items-center gap-2"><FileText className="h-12 w-12 text-[#C48830]/50" /></div>}
            </div>
            <div className="p-5 flex flex-col gap-1.5">
                <h3 className="font-semibold text-[#F0E8D8] text-sm">{doc.title}</h3>
                <p className="text-[#C0A880] text-xs leading-relaxed line-clamp-2">{doc.description}</p>
            </div>
        </motion.div>
    );
}
...
function DocumentViewer({ documents, initialIndex, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-[#0A0806]/95 backdrop-blur flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#C0A880]/10 flex-shrink-0">
                <div><h3 className="font-semibold text-[#F0E8D8]">{doc.title}</h3>{documents.length > 1 && <p className="text-xs text-[#80685A] mt-0.5">{index + 1} / {documents.length}</p>}</div>
                <button onClick={onClose} className="rounded-xl border border-[#C0A880]/20 p-2 hover:bg-[#221408]/35 transition"><X className="h-5 w-5 text-[#F0E8D8]" /></button>
            </div>
            <div className="flex-1 relative overflow-hidden">
                ...
                {documents.length > 1 && index > 0 && <button onClick={() => setIndex(i => i - 1)} className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full bg-[#0A0806]/80 backdrop-blur p-3 hover:bg-[#221408] transition"><ChevronLeft className="h-6 w-6 text-[#F0E8D8]" /></button>}
                {documents.length > 1 && index < documents.length - 1 && <button onClick={() => setIndex(i => i + 1)} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full bg-[#0A0806]/80 backdrop-blur p-3 hover:bg-[#221408] transition"><ChevronRight className="h-6 w-6 text-[#F0E8D8]" /></button>}
            </div>
            <div className="px-6 py-4 border-t border-[#C0A880]/10 flex-shrink-0 bg-[#0A0806]/80"><p className="text-[#C0A880] text-sm leading-relaxed">{doc.description}</p></div>
        </div>
    );
}

function SectionTitle({ label, title }) {
    return <div className="mb-8"><p className="text-xs text-[#80685A] uppercase tracking-widest mb-2">{label}</p><h2 className="text-2xl font-semibold tracking-tight">{title}</h2></div>;
}

function Sidebar({ ui, nav, duration }) {
    return (
        <aside className="hidden lg:flex flex-col gap-1 w-52 flex-shrink-0 sticky top-24">
            <p className="text-xs text-[#80685A] uppercase tracking-widest mb-3 px-3">{PROJECT_BASE.title}</p>
            {nav.map((item) => (
                <a key={item.href} href={item.href} className="px-3 py-2 rounded-xl text-sm text-[#C0A880] hover:text-[#F0E8D8] hover:bg-[#221408]/30 transition">{item.label}</a>
            ))}
            <div className="mt-6 px-3">
                <div className="h-px bg-[#C0A880]/10 mb-4" />
                <div className="flex flex-col gap-2 text-xs text-[#80685A]">
...
export default function ProjetFigureOut() {
    ...
    return (
        <div className="min-h-screen bg-[#0A0806] text-[#F0E8D8]" style={{ position: "relative" }}>
            <StarField />
            <div style={{ position: "relative", zIndex: 1 }}>
                <div className="sticky top-0 z-50 backdrop-blur bg-[#0A0806]/70 border-b border-[#C0A880]/10">
                    <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
                        <Link href="/" className="inline-flex items-center gap-2 text-[#80685A] hover:text-[#F0E8D8] transition text-sm"><ChevronLeft className="h-4 w-4" /> {t.backLink}</Link>
                        <div className="flex items-center gap-4">
                            <span className="font-semibold text-[#F0E8D8] hidden md:block">{PROJECT_BASE.title}</span>
                            <LangToggle />
                        </div>
                    </div>
                </div>
                <div className="mx-auto max-w-7xl px-6 pt-10 pb-0">
                    <motion.div variants={fadeUp} initial="initial" animate="animate" className="grid md:grid-cols-2 gap-4">
                        <div className="rounded-3xl overflow-hidden border border-[#C0A880]/10 h-80">
                    ...
                    <main className="flex-1 min-w-0 flex flex-col gap-16">
                        ...
                        <section id="general" className="scroll-mt-24">
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                <motion.div variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: true }} className="rounded-3xl border border-[#C0A880]/10 bg-[#221408]/20 p-7 flex flex-col gap-3">
                                    <h3 className="text-sm font-semibold text-[#80685A] uppercase tracking-widest">{ui.gameSummary}</h3>
                                    <p className="text-[#C0A880] text-sm leading-relaxed">{p.summary}</p>
                                </motion.div>
                                <motion.div variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: true }} className="rounded-3xl border border-[#C0A880]/10 bg-[#221408]/20 p-7 flex flex-col gap-4">
                                    <h3 className="text-sm font-semibold text-[#80685A] uppercase tracking-widest">{ui.devInfo}</h3>
                                    {[{ label: ui.teamSizeLabel, value: `${PROJECT_BASE.teamSize} ${ui.people}` }, { label: ui.durationLabel, value: p.duration }, { label: ui.engineLabel, value: PROJECT_BASE.engine }, { label: ui.platformLabel, value: PROJECT_BASE.platform }].map((info, i) => (
                                        <div key={i} className="flex justify-between items-center border-b border-[#C0A880]/10 pb-2 last:border-0 last:pb-0">
                                            <span className="text-xs text-[#80685A]">{info.label}</span>
                                            <span className="text-sm font-medium text-[#F0E8D8]">{info.value}</span>
                                        </div>
                                    ))}
                                </motion.div>
                            </div>
                            <motion.div variants={stagger} initial="initial" whileInView="animate" viewport={{ once: true }} className="grid md:grid-cols-3 gap-6 mt-6">
                                {p.keyFeatures.map((f, i) => (
                                    <motion.div key={i} variants={fadeUp} className="rounded-3xl border border-[#C0A880]/10 bg-[#221408]/20 p-6 flex flex-col gap-3">
                                        <h3 className="text-sm font-semibold text-[#F0E8D8]">{f.title}</h3>
                                        <p className="text-[#C0A880] text-sm leading-relaxed">{f.description}</p>
                                    </motion.div>
                                ))}
                            </motion.div>
                        </section>

                        <div className="h-px bg-gradient-to-r from-transparent via-[#C0A880]/20 to-transparent" />

                        <section id="roles" className="scroll-mt-24">
                            <SectionTitle label={ui.contribution} title={ui.rolesTitle} />
                            <div className="flex flex-col gap-6">
                                <motion.div variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: true }} className="rounded-3xl border border-[#C0A880]/10 bg-[#221408]/20 p-7">
                                    <div className="flex items-center gap-3 mb-5">
                                        <div className="rounded-xl bg-[#C48830]/15 p-2.5"><Users className="h-5 w-5 text-[#C48830]" /></div>
                                        <div><p className="text-xs text-[#80685A] uppercase tracking-widest">{ui.mainRoleLabel}</p><h3 className="font-semibold text-[#F0E8D8]">{p.mainRole.title}</h3></div>
                                    </div>
                                    <ul className="space-y-2.5 text-[#C0A880] text-sm list-disc pl-5">{p.mainRole.items.map((item, i) => <li key={i}>{item}</li>)}</ul>
                                </motion.div>
                                {p.secondaryRole && (
                                    <motion.div variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: true }} className="rounded-3xl border border-[#C0A880]/10 bg-[#221408]/20 p-7">
                                        <div className="flex items-center gap-3 mb-5">
                                            <div className="rounded-xl bg-[#C48830]/15 p-2.5"><Users className="h-5 w-5 text-[#C48830]" /></div>
                                            <div><p className="text-xs text-[#80685A] uppercase tracking-widest">{ui.secondaryRoleLabel}</p><h3 className="font-semibold text-[#F0E8D8]">{p.secondaryRole.title}</h3></div>
                                        </div>
                                        <ul className="space-y-2.5 text-[#C0A880] text-sm list-disc pl-5">{p.secondaryRole.items.map((item, i) => <li key={i}>{item}</li>)}</ul>
                                    </motion.div>
                                )}
                            </div>
                        </section>
                                ...
                                {p.analysis.map((block, i) => (
                                    <motion.div key={i} variants={fadeUp} initial="initial" whileInView="animate" viewport={{ once: true, amount: 0.1 }} className={`grid ${block.image ? "md:grid-cols-[3fr_2fr]" : "grid-cols-1"} gap-8 items-start`}>
                                        <div className="flex flex-col gap-4">
                                            <h3 className="text-lg font-semibold text-[#F0E8D8]">{block.heading}</h3>
                                            <p className="text-[#C0A880] leading-relaxed text-sm">{block.text}</p>
                                        </div>
                                        {block.image && (
                                            <div className="flex flex-col gap-2">
                                                <div className="rounded-xl overflow-hidden border border-[#C0A880]/10"><Image src={block.image} alt={block.imageCaption || block.heading} width={500} height={360} className="w-full object-cover" /></div>
                                                {block.imageCaption && <p className="text-xs text-[#80685A]/80 text-center italic">{block.imageCaption}</p>}
                                            </div>
                                        )}
                                    </motion.div>
                                ))}
                        ...
                        <div className="flex flex-wrap justify-between items-center gap-4 pt-4">
                            <Link href="/projets/gecko-pulco" className="inline-flex items-center gap-2 rounded-xl border border-[#C0A880]/20 px-5 py-2.5 text-sm hover:bg-[#221408]/35 transition"><ChevronLeft className="h-4 w-4" /> {ui.prevProject}</Link>
                            <Link href="/projets/10-nichi" className="inline-flex items-center gap-2 rounded-xl bg-white text-[#120A04] px-5 py-2.5 font-medium hover:bg-[#F0E8D8] transition text-sm">{ui.nextProject}</Link>
                        </div>
                    </main>
                </div>
                <footer className="border-t border-[#C0A880]/10 mt-8">
                    <div className="mx-auto max-w-7xl px-6 py-8 flex flex-wrap items-center justify-between gap-4 text-sm text-[#80685A]">
                        <span>© {new Date().getFullYear()} Lou-Ann Barry.</span>
                        <div className="flex items-center gap-4">
                            <a href={LINKS.github} className="hover:text-[#F0E8D8] transition"><Github className="h-4 w-4" /></a>
                            <a href={LINKS.linkedin} className="hover:text-[#F0E8D8] transition"><Linkedin className="h-4 w-4" /></a>
                            <a href={`mailto:${LINKS.email}`} className="hover:text-[#F0E8D8] transition"><Mail className="h-4 w-4" /></a>
                        </div>
                    </div>
                </footer>
            </div>
